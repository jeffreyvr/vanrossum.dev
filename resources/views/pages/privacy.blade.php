@extends('layouts.app')

@section('content')
<div class="bg-white py-12 relative">
    <article class="max-w-xl md:max-w-3xl mx-auto px-8 md:px-0">
        <header class="mb-8">
            <h1 class="text-5xl font-extrabold leading-tight mb-1">{{ __('Privacy statement') }}</h1>
        </header>

        <div class="text-xl leading-relaxed markup">
            @markdown
            The following applies to this website vanrossum.dev.

            ## What are cookies?
            A cookie is a small (text) file that is stored on your computer by your browser when you visit our website.
            This information can be sent back to us on your next visit. This way our website can recognize you.

            ## Which cookies do we use?
            Functional cookies: These cookies are necessary for the website to work. Without these cookies you and some
            preferences (such as your language settings) cannot be saved.
            Google Analytics: We use Google Analytics to test the quality and effectiveness of our website. This way we
            can see how the website is used and how we can improve the website. The data is anonymous and therefore
            cannot be traced back to a specific person.
            @endmarkdown
        </div>

    </article>
</div>
@endsection